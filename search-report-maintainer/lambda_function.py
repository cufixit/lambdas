...
def lambda_handler(event, context):
    ...
    if path_params := event["pathParameters"]:
        reportID = path_params.get("reportId") #note that the "d" is not capalized in reportId
        
    try:
        
        if reportID:
            print("You should not be making into this part if you are testing empty query")
            user_query = reportID
            dynamoIDs = query(user_query) # this is returning multiple results because of the way I was generating test cases based on timestamp
