...
def send_queue(txt):
    ...
    sqs_response = sqs.send_message(QueueUrl=KEYWORD_Q, MessageBody=json.dumps(txt))
    return sqs_response
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(e)
        print(
            "Error getting object {} from queue. Make sure they exist and your bucket is in the same region as this function.".format(
                reportID
            )
        )
        raise e

    txt = {"reportID": reportID, "description": description}
    sqs_response = send_queue(txt)
    print(sqs_response)

    return {"statusCode": 200, "body": json.dumps("Successfully indexed report")}
